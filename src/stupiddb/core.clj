...
(defn- handle-log [db [action key value]]
  (case action
    :assoc    (update-in db [:data] assoc key value)
    :dissoc   (update-in db [:data] dissoc key)
    :assoc-in (update-in db (vec (concat [:data] key)) (constantly value))
    db))
